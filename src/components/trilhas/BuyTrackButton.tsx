...
export function BuyTrackButton({ trackSlug, price, isAuthenticated }: Props) {
  ...
  async function handleBuy() {
    if (!isAuthenticated) {
      router.push(`/login?next=${encodeURIComponent(`/trilhas/${trackSlug}`)}`)
      return
    }
    ...
    try {
      const res = await fetch(`/api/tracks/${trackSlug}/purchase`, { method: 'POST' })
  ...
  return (
    <div className="flex flex-col items-center gap-1.5">
      ...
      >
        {loading
          ? 'Aguarde...'
          : `Comprar trilha — R$ ${price.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}`}
      </button>
