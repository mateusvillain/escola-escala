...
> A Fase 1 (seções 1–10 abaixo) e a Fase 2 (seção 13, TASK-84 a TASK-145) estão implementadas — ver `CLAUDE.md`/`AGENTS.md` do repositório para o estado real de cada funcionalidade. A seção 14 detalha a Fase 3 (TASK-146 a TASK-195).
...
Detalhada na seção 14. Resumo por sprint sugerido (ordem de prioridade definida na seção 14 abaixo):
...
- Boleto via Stripe na compra avulsa, com tratamento de pagamento assíncrono (Pix adiado — ver `docs/wiki/pix-habilitacao.md`)
...
Levantamento original de oportunidades, seções 1–5 (seção 6, B2B e fórum, permanece fora de escopo — ver seção 12). Cada requisito referencia o ID de task correspondente em `.agent/tasks/`.
...
Esta seção converte o levantamento original de oportunidades (7 itens, que não previa quebra em tasks até validação com o cliente) em tasks de implementação, mais uma frente adicional (14.7) decidida em conversa com o cliente em 2026-06-21, fora do escopo original do roadmap. **B2B (14.1) foi despriorizado e suas tasks (TASK-151 a TASK-166) movidas para `.agent/tasks/deprecated/`** — ver decisão na seção 14.6; o spec permanece descrito abaixo (14.1/14.4) como referência para quando a direção for revisitada, mas não faz parte dos sprints ativos (seção 11). **Boleto (14.1) foi removido do planejamento ativo e suas tasks (TASK-147 a TASK-150) também movidas para `.agent/tasks/deprecated/`** — ver decisão na seção 14.6. Cada requisito referencia o ID de task correspondente em `.agent/tasks/`.

### 14.1 Monetização e Expansão de Mercado (TASK-167 a TASK-173 ativas; TASK-147 a TASK-150 e TASK-151 a TASK-166 despriorizadas)

**Boleto via Stripe — `[REMOVIDO, ver 14.6]`** — TASK-147 a TASK-150 (movidas para `.agent/tasks/deprecated/`): cobriria Boleto na Checkout Session de compra avulsa (`mode: 'payment'`), com tratamento dos webhooks de pagamento assíncrono (`checkout.session.async_payment_succeeded`/`async_payment_failed`) e estado de "pagamento pendente" na UI, além de investigação sobre Boleto na assinatura recorrente. Removido do planejamento ativo da Fase 3 em 2026-06-21 — Pix segue adiado pelo motivo original (convite da Stripe pendente, ver `docs/wiki/pix-habilitacao.md`), mas agora nenhum dos dois métodos alternativos a cartão está em implementação ativa.
...
**Trilhas e bundles de cursos** — TASK-167 a TASK-173: `CourseTrack`/`CourseTrackItem` agrupam cursos em sequência; trilha pode ser vendida como bundle em pagamento único (reaproveitando o padrão de compra avulsa individual, só cartão), com matrícula em lote em todos os cursos da trilha via webhook.
...
| Convite da Stripe para habilitar Pix (BRL/BR) | Reabilitar Pix na compra avulsa/bundle — adiado da Fase 3 | Adiado — ver `docs/wiki/pix-habilitacao.md` para solicitar |
...
- **B2B (despriorizado em 2026-06-21):** demanda de clientes corporativos ainda não validada — o levantamento original de oportunidades classificava B2B como "maior alavanca de receita do roadmap atual", mas recomendava "validação de demanda antes de investir". Decisão tomada: em vez de manter as tasks no sprint ativo só com essa ressalva, TASK-151 a TASK-166 foram movidas para `.agent/tasks/deprecated/` e retiradas dos sprints da seção 11. O spec completo permanece intacto (descrito em 14.1/14.4) para retomada futura, condicionado a validar a demanda com o cliente antes de reabrir qualquer um dos Grupos 2 a 5 (`docs/fase3-grupos-de-trabalho.md`).
...
- **Boleto (removido em 2026-06-21):** durante a implementação do checkout integrado ao cadastro (seção 14.7), identificou-se que `handleCheckoutSessionCompleted` (`src/lib/stripe-handlers.ts`) assume que toda Checkout Session de assinatura completada já foi paga (`subscriptionStatus = trialing ? trialing : active`) — o que é falso para Boleto, cujo `checkout.session.completed` dispara na *emissão* do boleto, não no pagamento (status real fica `incomplete` até a compensação, 1-3 dias úteis). Habilitar Boleto sem o tratamento assíncrono completo (TASK-147/148/150) concederia acesso pago antes da confirmação do pagamento. Decisão do cliente: em vez de implementar esse tratamento agora, Boleto foi removido do planejamento ativo da Fase 3 — desabilitado diretamente na conta Stripe (Dashboard), e TASK-147 a TASK-150 movidas para `.agent/tasks/deprecated/`. TASK-172/173 (bundle de trilha) ajustadas para usar só cartão, sem mais depender de TASK-147/148. Spec preservado para retomada futura, condicionado a implementar o tratamento assíncrono completo antes de reabilitar.
- Pix: continua adiado pelo motivo original (convite da Stripe pendente para contas BR, ver `docs/wiki/pix-habilitacao.md`) — independente da remoção de Boleto, que teve motivo diferente (gap de tratamento assíncrono, não bloqueio externo)
...
Decidido em 2026-06-21, fora do escopo original do roadmap de Fase 3. Hoje o cadastro (`/cadastro`) só cria a conta e redireciona para `/dashboard` — a escolha de plano acontece numa visita separada a `/planos`, e quem ainda não tem conta e clica em "Assinar" lá é mandado para `/login`, sem caminho direto para criar conta preservando a intenção de assinatura. Esta frente junta os dois passos numa página só, usando o **Embedded Checkout** do Stripe (`ui_mode: 'embedded'`) em vez do Checkout hospedado — o formulário de pagamento é renderizado num iframe dentro da própria página, sem redirecionar o usuário para `checkout.stripe.com`.
...
- **TASK-199** — `RegisterForm` ganha passos pós-registro: com plano pré-selecionado (vindo de `/planos`), abre o Embedded Checkout inline direto; **sem plano pré-selecionado (cadastro "normal", escopo ampliado em 2026-06-21)**, mostra um seletor com 3 cards — Gratuito, Básico, Premium — antes de seguir.
...
**Decisão de escopo:** não há custo adicional do Stripe por usar Embedded em vez de Hosted Checkout — é o mesmo produto Checkout Session, mesma tabela de taxas por forma de pagamento (`docs/pricing/stripe-pricing.md`), só muda onde a UI é renderizada. Esta frente cobre apenas o checkout de **assinatura** (`mode: 'subscription'`, planos Básico/Premium) integrado ao cadastro — não estende Embedded Checkout à compra avulsa de curso nem ao bundle de trilha (TASK-167 a 173) nesta rodada.

**Plano Gratuito (adicionado em 2026-06-21):** todo cadastro que não chega com um plano pré-selecionado agora passa por um seletor de plano antes do dashboard, com uma terceira opção "Gratuito" — escolher essa opção pula o checkout e vai direto para `/dashboard`, sem criar nenhuma assinatura. Não é um conceito novo de backend: é equivalente ao estado já existente de usuário autenticado sem `UserSubscription` (acesso só a aulas `isPreview: true`, via `src/lib/access.ts`, inalterado). Nenhum model, campo ou enum novo foi criado no Prisma para isso.
