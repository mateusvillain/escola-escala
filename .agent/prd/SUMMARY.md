...
## Fase 3 — Expansão e Maturidade (planejada, TASK-146 a TASK-150 e TASK-167 a TASK-195 ativas)

A Fase 3 ativa soma 34 tasks em 3 frentes: monetização e expansão de mercado (Boleto via Stripe na compra avulsa — Pix adiado, ver `docs/pix-habilitacao.md` —, trilhas e bundles de cursos), conteúdo (liberação programada/drip content por módulo) e confiabilidade e dados de produto (testes E2E com Playwright, rate limiting distribuído via Upstash Redis, monitoramento de erro com Sentry, event tracking de produto). Detalhes em `.agent/prd/PRD.md` seção 14 e na origem em `docs/fase3-oportunidades.md`. **B2B/licenciamento corporativo (TASK-151 a TASK-166) foi despriorizado em 2026-06-21** e movido para `.agent/tasks/deprecated/` — era a maior aposta de receita do grupo, mas ainda sem demanda validada com o cliente; o spec permanece pronto para retomada futura (PRD seção 14.6). Fórum, app mobile nativo e gamificação competitiva permanecem fora de escopo (Fase 4+).
